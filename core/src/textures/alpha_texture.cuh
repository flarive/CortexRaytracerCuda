...
__host__ __device__ alpha_texture::alpha_texture(texture* alpha, bool double_sided = false) : m_alpha(alpha), m_double_sided(double_sided)
{
}

__host__ __device__ color alpha_texture::value(float u, float v, const point3& p) const
{
    return m_alpha->value(u, v, p);
}

__host__ __device__ bool alpha_texture::is_double_sided() const
{
