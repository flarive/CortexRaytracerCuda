...
__host__ __device__ inline normal_texture::normal_texture(texture* normal, float strength) : m_normal(normal), m_strength(strength)
{
}

__host__ __device__ inline color normal_texture::value(float u, float v, const point3& p) const
{
...
__host__ __device__ inline float normal_texture::getStrenth()
{
    return m_strength;
}
