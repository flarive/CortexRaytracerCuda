...
public:
	...
	__device__ virtual vector3 generate_samples(int s_i, int s_j, thrust::default_random_engine& rng) const override;
};
...
/// <returns></returns>
__device__ inline vector3 random_sampler::generate_samples(int s_i, int s_j, thrust::default_random_engine& rng) const
{
    // Generate random positions within the pixel
    float px = -0.5f + m_recip_sqrt_spp * (s_i + get_real(rng));
    float py = -0.5f + m_recip_sqrt_spp * (s_j + get_real(rng));
