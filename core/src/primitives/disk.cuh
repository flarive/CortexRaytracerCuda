...
__host__ __device__ inline disk::disk(const char* _name)
    : disk(vector3(0, 0, 0), 1.0, 2.0, nullptr, uvmapping(), _name)
{
}

__host__ __device__ inline disk::disk(point3 _center, float _radius, float _height, material* _mat, const char* _name)
    : disk(_center, _radius, _height, _mat, uvmapping(), _name)
{
}

__host__ __device__ inline disk::disk(point3 _center, float _radius, float _height, material* _mat, const uvmapping& _mapping, const char* _name)
...
__device__ inline bool disk::hit(const ray& r, interval ray_t, hit_record& rec, int depth, int max_depth, thrust::default_random_engine& rng) const
{
...
__device__ inline float disk::pdf_value(const point3& o, const vector3& v, int max_depth, thrust::default_random_engine& rng) const
{
    return 0.0f;
}

__device__ inline vector3 disk::random(const vector3& o, thrust::default_random_engine& rng) const
{
    return vector3(1, 0, 0);
}

__host__ __device__ inline aabb disk::bounding_box() const
{
    return m_bbox;
}