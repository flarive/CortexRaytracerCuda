...
//        scattered = ray(rec.hit_point, reflected + fuzz * random_in_unit_sphere(rng));
//        attenuation = albedo;
...
__device__ bool metal::scatter(const ray& r_in, const hittable_list& lights, const hit_record& rec, scatter_record& srec, thrust::default_random_engine& rng) const
{
    ...
    srec.skip_pdf_ray = ray(rec.hit_point, reflected + m_fuzz * random_in_unit_sphere(rng), r_in.time());
    return true;
}