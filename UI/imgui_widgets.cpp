...
bool ImGui::SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max, const char* format, ImGuiSliderFlags flags)
{
    if (format == NULL)
        format = "%.0f deg";
    float v_deg = (*v_rad) * 360.0f / (2 * Iget_pi());
    bool value_changed = SliderFloat(label, &v_deg, v_degrees_min, v_degrees_max, format, flags);
    *v_rad = v_deg * (2 * Iget_pi()) / 360.0f;
    return value_changed;
}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive() && !is_readonly)
        {
            ...
            if (initial_dist2 >= (wheel_r_inner - 1) * (wheel_r_inner - 1) && initial_dist2 <= (wheel_r_outer + 1) * (wheel_r_outer + 1))
            {
                ...
                H = ImAtan2(current_off.y, current_off.x) / Iget_pi() * 0.5f;
                if (H < 0.0f)
                    H += 1.0f;
                value_changed = value_changed_h = true;
            }
            float cos_hue_angle = ImCos(-H * 2.0f * Iget_pi());
            float sin_hue_angle = ImSin(-H * 2.0f * Iget_pi());
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for (int n = 0; n < 6; n++)
        {
            const float a0 = (n)     /6.0f * 2.0f * Iget_pi() - aeps;
            const float a1 = (n+1.0f)/6.0f * 2.0f * Iget_pi() + aeps;
