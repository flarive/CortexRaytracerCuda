...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    if (table->InstanceCurrent == 0)
    {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0)
        {
            ...
            // In the scene_loader below, moving C to the right of E will lead to:
...
void ImGui::TableAngledHeadersRowEx(float angle, float max_label_width)
{
    ...
    // Angle argument expressed in (-Iget_pi()/2 .. +Iget_pi()/2) as it is easier to think about for user.
    const bool flip_label = (angle < 0.0f);
    angle -= Iget_pi() * 0.5f;
    const float cos_a = ImCos(angle);
    const float sin_a = ImSin(angle);
    const float label_cos_a = flip_label ? ImCos(angle + Iget_pi()) : cos_a;
    const float label_sin_a = flip_label ? ImSin(angle + Iget_pi()) : sin_a;
